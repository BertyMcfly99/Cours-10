...
tables_pipes <-  read_html(x=URL) |> 
  html_elements(css="table") |> 
  html_table()

nouveau_table <- dplyr::bind_rows(tables_pipes[[1]], tables_pipes[[2]], tables_pipes[[3]], 
                                  .id="type")

#Met le S dans elements Bertrand
```

Remplir les rangées de la colonne

Province/territoire

```{r}
table |> tidyr::fill("Province/Territoire", .direction = "down")

table <- nouveau_table |> dplyr::rename(province_territoire = "Province/Territoire", aeroport = "Aéroport")

x [x = ""] <- NA

table |> dplyr::na_if(province_territoire, y = "")

table[table = ""] <- NA

#Combiner les colonnes province_tierrtoire et territoire) 
table$province_territoire <- dplyr::coalesce(table$province_territoire,
                                             table$Territoire)

#Remplacer "" avec NA dans la colonne province_territoire
table$province_territoire <- table$province_territoire |> dplyr::na_if(y="")

#Fill les lignes manquantaes avec le province en direction down
table <-  table |>  tidyr::fill(province_territoire, .direction = "down")

#Enlever une colonne
table |> dplyr::select(-Territoire)

#Permet de venir créer une nouvelle colone avec 
x <- table |>  mutate(new_col = str_extract_all(string = aeroport, pattern = "[A-Z]{3}"))

table |>  tidyr::extract(col = aeroport,
                        into = "code_aeroport",
                        regex = "(Y[A-Z]{3})",
                        remove = FALSE)


table$aeroport |> stringr::str_remove_all(pattern = " \\(.*\\)")

#Aller voir sur regex pour savoir comment bien enlever les lettres.
# regular expression




# Chanement en texte des types d'aéroport

x <- table |> mutate(type_nouveau = recode(type, "1" = "Petits aéroports","2" = "Aéroports nationaux", "3" = "Aéroports Nationaux par des administrations territoriales"))
```
